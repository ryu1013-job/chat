  ...
  return (
    <Disclosure
      ...
      className={composeTailwindRenderProps(
        className,
        twMerge('px-0', state !== 'collapsed' && 'col-span-full'),
      )}
      {...props}
    />
  )
...
function SidebarDisclosurePanel(props: React.ComponentProps<typeof DisclosurePanel>) {
  return (
    <DisclosurePanel
...
function SidebarTrigger({ onPress, children, ...props }: React.ComponentProps<typeof Button>) {
...
function SidebarLabel({ className, ref, ...props }: SidebarLabelProps) {
  const { state, isMobile } = useSidebar()
  const collapsed = state === 'collapsed' && !isMobile
  if (!collapsed) {
    return (
